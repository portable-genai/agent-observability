# Compliance map: `agent-observability` Agent Observability, Audit & FinOps

How `agent-observability` maps to the platform's general principles (P-01..P-12) and rules (R1..R6). `agent-observability` is a
narrow system of record, so several principles are owned upstream (`agent-guardrail-gateway` redaction) or are
not applicable; those are marked honestly rather than padded.

## Rules

| Rule | Requirement | `agent-observability` control |
|---|---|---|
| **R1** | Redaction at the boundary | Enforced upstream by `agent-guardrail-gateway`. `agent-observability` only ever receives `redacted_prompt` / `redacted_response`; its store has no raw-PII column, and the `redaction_preserved` eval check proves only redacted text is persisted. |
| **R2** | Immutable (WORM) audit | The audit store is Write-Once-Read-Many. `gcp`: a Cloud Logging bucket with `locked = var.worm_locked`, a variable with no default, so every deployment names the lock; a production deployment sets it true at 2557 days (~7y), encrypted with the bank's own CMEK. `local`: an append-only, hash-chained SQLite stand-in. Append-only is enforced by SQLite triggers, every record is chained (`entry_hash = SHA-256(prev_hash || "\n" || event_json)`), the head and the retention-prune watermark are anchored in an external file, and `agent-observability audit verify | export | restore` proves it. `src/observability/adapters/local/audit.py` states exactly which tamper classes are and are not detected. `onprem`: a placeholder that raises rather than dropping a record. Callers of the ingest are themselves authenticated (`require_service_caller`, SPEC §6.1): `gcp` a Google-signed OIDC ID token (audience + caller allowlist), `local` a constant-time shared secret; `/healthz` stays open. So only enrolled platform services can write to or read the trail, hardening the separation that stops a caller tampering with its own audit record. |
| **R3** | Agent registration / discovery | n/a for `agent-observability` (owned by `agent-registry`). |
| **R4** | Eval / model-risk gate | `model-quality-gate`-style offline gate (`eval/run_eval.py`) checks audit-trail integrity (parity, provenance, redaction, ordering) on every change. |
| **R5** | Maker-checker on consequential output | n/a: `agent-observability` produces no agent output; it records the maker-checker `decision` (`escalated`) emitted upstream. |
...
| Principle | `agent-observability` control |
|---|---|
| **P-01** human-in-the-loop | n/a directly: `agent-observability` stores the upstream `decision` (`allowed` / `blocked` / `escalated`) so a reviewer can audit maker-checker outcomes. |
...
| **P-11** observability | `agent-observability` *is* the platform's observability system: tracing (OTel collector), audit (this service), FinOps (BigQuery export). |
...
**Ownership: the adopting institution owns this section, not upstream `agent-observability`.** The rows below
...
| MAS reference | Obligation (adopter's own reading) | `agent-observability` evidence |
|---|---|---|
| MAS 626 / TRM Guidelines 11 | Audit trail of system activity, retained and protected from modification | Cloud Logging bucket locked by `worm_locked = true` in a production deployment, 2557-day retention (`infra/terraform/logging_worm.tf`); hash-chained offline stand-in with `audit verify` |
...
| MAS 626 (record retention) | Retention period appropriate to the record class | `retention_days` variable, defaulted to ~7 years; the floor binds when `worm_locked = true`, which has no default and is adopter-set before the irreversible lock |
